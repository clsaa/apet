...
- **M3**（大体完成）：
  ...
  - **M3-C++ Codex 接入**（2026-07-05，见 `docs/superpowers/specs/2026-07-05-apet-codex-source-design.md`）：`~/.codex/sessions/**/rollout-*.jsonl`（首行 session_meta）+ `session_index.jsonl` thread_name 白送标题；task_started/complete 轮次信号经 `CodexRolloutParse` 映射 end_turn，**全复用** JSONLDirectoryWatcher/Scanner；合成新鲜会话真机门验证 upsert。resume 未核实→不提供；rollout schema 非 Claude 同构→摘要菜单隐藏（`claudeStyleTranscriptAgents` 门控）。
