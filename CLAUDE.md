...
- **M3**（大体完成）：
  ...
  - **M3-C++ Codex 接入**（2026-07-05，见 `docs/superpowers/specs/2026-07-05-apet-codex-source-design.md`）：`~/.codex/sessions/**/rollout-*.jsonl`（首行 session_meta）+ `session_index.jsonl` thread_name 白送标题；task_started/complete 轮次信号经 `CodexRolloutParse` 映射 end_turn，**全复用** JSONLDirectoryWatcher/Scanner；合成新鲜会话真机门验证 upsert。**resume 已核实提供**（codex-cli 0.142.5 实跑：`codex resume <UUID>`）；rollout schema 非 Claude 同构→摘要菜单隐藏（`claudeStyleTranscriptAgents` 门控）；三视角评审修复（turn_aborted/长轮次截断兜底/noJumpAgents 诚实降级/DisplaySanitizer 标题消毒）；**Desktop/CLI 分流**（首条 meta source 分 codex-desktop/codex，desktop 点击激活 Codex.app，身份取首条防混用漂移，真机门验证；34 测试）。
