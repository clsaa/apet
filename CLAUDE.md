...
- **M3**（大体完成）：
  ...
  - **M3-C 多 Agent** ✅（AgentManifest 契约 + **QoderWork 实测接入**：SQLite agents.db 只读轮询/粗略态/点击激活 App + **Qoder CLI 实测接入**：~/.qoder jsonl 与 Claude 同构/resume=`qodercli --resume`/**hook 精确跳转**（2026-07-05：官方 `qodercli hooks migrate` 自认 Claude 兼容 → apet-emit-event.sh 经 AGENTPET_AGENT 参数化复用,首选项「Qoder CLI 精确跳转」门控装进 ~/.qoder/settings.json,与 Claude 同一 HookInstaller/备份/卸载链）。
  ...
  - **OpenCode 插件增强** ✅（2026-07-05,P1 落地）：`apet-opencode-notify.js` 全局插件（~/.config/opencode/plugins/,门控文件级安装:外人同名文件拒绝覆盖/删除）订阅 session.idle→stop、question.asked→attention、session.status busy→busy（实测 1.17.13 事件流）——OpenCode 获得精确跳转（tty/pid/iTerm id 进程内采集）+真 OS 通知+pid 存活保护+实时 running；root 烤入=dirname(opencode.db) 与 DB watcher 同键防会话分裂;真机门:项目级插件 opencode run 实跑事件落库。首选项第三个门控开关。
  - **发布工程+生态+通知/统计**（2026-07-06,E/F+C/D）:CI(push/PR swift test @macos-14,首跑绿)+tag Release 流水线(ad-hoc 签名,正式签名路径见 docs/RELEASING.md);第三方接入指南 docs/INTEGRATING.md(事件契约+三模式+四家样例);通知副标题带手动摘要(与标题去重);首选项「会话统计」(历史索引聚合:各 agent 近7天/累计)。
  - **事件日志压缩+键盘流**（2026-07-06）：启动压缩 events.ndjson(EventLogCompactor:近72h或pid存活会话尾50条+终端载体,行序=seq序;>5000行触发原子写;真机 13972→212 行)；面板键盘流(↑↓选行高亮滚动跟随/回车跳转/Esc关面板/⌘F聚焦搜索;NSEvent本地监视器,文本编辑中放行)。
  - **历史档案+手动摘要**（2026-07-06,见 `docs/superpowers/specs/2026-07-06-apet-history-and-notes-design.md`）：面板「历史」tab=全量磁盘索引（claude/qoder×2 jsonl + codex session_index/rollout + opencode DB;HistoryIndexer (path,mtime) 缓存零重析,30s 节流异步构建;历史行右键动作全可用,sessionForId 回退历史;桌宠侧不接=provider nil 自动隐藏 tab）；`SessionMeta.note` 手动摘要（行内编辑与重命名同交互,✎ 显示副标题位路径退 tooltip,可搜索,摘要 banner「存为手动摘要」一键转正,≤200 字）。
  ...
  - 真实遗留：Qoder IDE 追加接入（**搁置**：产品线合并未定）、模型摘要 UI、F10 createdAt 注入、**OpenCode 上游维护义务**（watch sst/opencode releases 与 `packages/core/src/database/` 变更；版本漂移按 `OpenCodeDBReader.verifiedMaxMigrationId` doc 注释的四步流程更新——上游 ~8 迁移/月，无流程必然烂掉）。
