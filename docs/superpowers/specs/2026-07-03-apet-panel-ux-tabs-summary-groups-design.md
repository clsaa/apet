...
- 分组嵌套 / 分组图标颜色(YAGNI)。
- **E10 悬停 ⋯ 溢出入口**:✅ 已做(用户批准后补)——悬停行 ☆ 左侧露 `⋯` = 右键菜单同款(rowMenuItems 共享)。
- **可缩放窗口 hidesOnDeactivate 取舍**:✅ 已加**图钉**(用户批准后补)——页脚「固定」按钮 toggle `config.panelPinned`,固定则常驻不自隐,默认非固定(近 popover transient)。
